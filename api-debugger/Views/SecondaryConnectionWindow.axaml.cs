...
public partial class SecondaryConnectionWindow : Window, IDisposable
{
    private readonly SecondaryConnectionViewModel _viewModel;
    private bool _disposed;

    public SecondaryConnectionWindow() : this("127.0.0.1", 3000)
    {
    }

    public SecondaryConnectionWindow(string ipAddress, int port)
    {
        InitializeComponent();
        _viewModel = new SecondaryConnectionViewModel(ipAddress, port);
    ...
    {
        if (e.PropertyName == nameof(SecondaryConnectionViewModel.ResponseText))
        {
            UpdateResponseHighlighting();
        }
    }
    ...
    private void LogMessages_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        if (e.Action == NotifyCollectionChangedAction.Add && _viewModel.LogMessages.Count > 0)
        {
            ...
            LogListBox.ScrollIntoView(_viewModel.LogMessages[^1]);
        }
    }

    private void UpdateResponseHighlighting()
    {
        var responseText = _viewModel.ResponseText;
        if (string.IsNullOrEmpty(responseText))
        {
            ResponseTextBlock.Inlines?.Clear();
            return;
        }

        ApplyJsonSyntaxHighlighting(ResponseTextBlock, responseText);
    }
    ...
    private void LogList_DoubleTapped(object? sender, TappedEventArgs e)
    {
        ...
        var jsonData = SecondaryConnectionViewModel.GetJsonForMessage(selectedMessage);
